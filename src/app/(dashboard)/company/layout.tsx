  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Sidebar */}
        <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:static top-0 left-0 h-full w-64 bg-blue-600 dark:bg-gray-900 text-white p-6 transform transition-transform z-40 overflow-y-auto max-h-screen`}>          
          <button className="text-white mb-4 md:hidden" onClick={() => setSidebarOpen(false)}>✕ Close</button>
          <nav className="flex flex-col gap-4">
            {navLinks.map(link => (
              <Link
