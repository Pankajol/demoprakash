...
const columnOrder = [
  'VNo', 'MyType', 'UsrDate', 'PartyCode', 'Party', 'VAmt', 'CityArea',
  'ItemNo', 'AdjAmt', 'AdjIds', 'Trading', 'TradingLed', 'YearId',
  'UsrName', 'EditMode', 'EditUpdate', 'CDF', 'companyCode',
  'DrTypeId', 'CrTypeId'
];
...
const TransactionDetailsPage: React.FC = () => {
  ...
  const [partySelection, setPartySelection] = useState<Option | null>(
    initParty ? { value: initParty.split(' - ')[0], label: initParty } : null
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <DatePicker
              ...
              onChange={(d: Date | null) => setFilterFromDate(d || new Date())}
          ...
          <div>
            ...
            <DatePicker
              ...
              onChange={(d: Date | null) => setFilterToDate(d || new Date())}
              dateFormat="dd-MMM-yyyy"
              className="mt-1 w-full rounded border px-3 py-2"
              required
            />
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  ...
                  <tr className="bg-gray-50">
                    {columnOrder.map(header => {
                      const filterVal = {
                        MyType: filterMyType,
                        UsrDate: filterUsrDate,
                        PartyCode: filterPartyCode,
                        Party: filterParty,
                        CityArea: filterCityArea,
                      }[header] || '';

                      const setFilterFn = {
                        MyType: setFilterMyType,
                        UsrDate: setFilterUsrDate,
                        PartyCode: setFilterPartyCode,
                        Party: setFilterParty,
                        CityArea: setFilterCityArea,
                      }[header];

                      return (
                        <th key={`filter-${header}`} className="px-4 py-2">
                          {setFilterFn ? (
                            <input
                              type="text"
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredData.map((row, index) => (
                    <tr key={`row-${index}`}>
                      {columnOrder.map(key => (
                        <td key={`cell-${index}-${key}`} className="px-4 py-2 whitespace-nowrap text-sm text-gray-900">
                          {(key === 'UsrDate' || key === 'EditUpdate') && row[key]
